...
from tcga_encoder.utils.helpers import load_gmt
import numpy as np
...
class Pathways( object ):
  def __init__( self, kegg_location = "data/kegg/hsa_KEGG", hgnc_location = "data/hgnc_processed" ):
    ...
    print "Loading Hugo"
    self.LoadGenes()
    print "Loading Cancer Hallmarks"
    self.LoadHallmarks()
    # print "Loading Kegg"
    # self.LoadKegg()
    # print "Joining"
  ...
  def LoadHallmarks(self):
    self.pathway2hallmarkgenes, self.hallmarkgene2pathways = load_gmt( self.kegg_dir + "/h.all.v6.0.symbols.gmt" )
    #pdb.set_trace()  
    #self.sets_kgmls = pd.read_csv( self.kegg_dir + "/h.all.v6.0.symbols.gmt", sep="\t", index_col="ID" )
  ...
  def CancerEnrichment( self, hugo_list, weights = None ):
    ...
    pathway_count = OrderedDict()
    for p in self.pathway2hallmarkgenes.keys():
      pathway_count[p] = 0
    for hugo,w in zip( hugo_list, weights ):
      
      if self.hallmarkgene2pathways.has_key( hugo ):
        total_w+=w
        pathways = self.hallmarkgene2pathways[ hugo ]
        path_weights = w*np.ones(len(pathways)) #/len(pathways)
        ...
        for pathway, p_weight in zip( pathways, path_weights ):
          if self.pathway2hallmarkgenes.has_key( pathway ):
            c_pathways.append( pathway )
            c_weights.append(p_weight/len(self.pathway2hallmarkgenes[pathway]) )
            #has_cancer = True
            pathway_count[pathway] += p_weight/len(self.pathway2hallmarkgenes[pathway])
    ...
    most_common_readable = most_common #pd.Series( np.array(c.values())/len(hugo_list), index = self.sets_kgmls.loc[c.keys()]["Symbol"], name="readable" )
    #for mc in most_common:
    #  most_common_readable.append( (self.sets_kgmls.loc[mc[0]]["Symbol"], mc[1]) )    
    most_common = pd.Series( pathway_count.values(), index = pathway_count.keys(), name = "hallmark")
    return most_common # most_common.sort_values(ascending=False), most_common_readable.sort_values(ascending=False)
