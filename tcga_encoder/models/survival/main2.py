...
if __name__ == "__main__":
  ...
  for survival_spec in survival_dict:
    ...
    for data_name, data_info in data.iteritems():
      ...
      # # new importance
      # dim = reg_X.values.shape[1]
      # errors = np.zeros(dim)
      # corrs  = np.zeros(dim)
      # w_imp  = np.zeros( (dim,dim) )
      ...
      #   X[:,d_idx] = 0
      #
      #   m = BootstrapLassoRegression( dim, l1_regression )
      #   m.fit( X, y, n_bootstraps = 500 )
      #
      #   y_pred = m.predict( X )
      #
      #   corrs[d_idx] = np.corrcoef( y, y_pred )[0][1]
      #   errors[d_idx] = np.mean( np.square( y - y_pred ) )
      #   w_imp[d_idx]  = m.w
  pp.show()
