...
def replace_template(template, fold_idx,  n_xval_folds ):
  ...
  y['logging']['experiment'] += "/fold_%d_of_%d"%(fold_idx,n_xval_folds )
...
if __name__ == "__main__":
  ...
  fold=int(sys.argv[2])
  n_xval_folds=int(sys.argv[3])
  #run = bool(int( sys.argv[3]))
  #run = False
  #if len(sys.argv) == 3:
  ...
  yaml_file = replace_template( yaml_template_file, fold, n_xval_folds )
  # if run is True:
  #   runner( yaml_file )
