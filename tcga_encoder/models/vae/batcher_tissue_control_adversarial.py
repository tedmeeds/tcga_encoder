...
class TCGABatcherAdversarial( TCGABatcher ):
  ...
  def  PrintAndPlotDnaPredictions(self,sess, info_dict):
    ...
    ax2.plot( np.linspace(0,1,len(I_val)), 1.0-val_targets.values.flatten()[I_val].cumsum()/val_weights, 'r--' )
    ...
    ax2.plot( np.linspace(0,1,len(I_train)), 1.0- train_targets.values.flatten()[I_train].cumsum()/train_weights, 'b--' )
