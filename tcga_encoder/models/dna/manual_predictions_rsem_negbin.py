...
if __name__ == "__main__":
  ...
  for dna_gene in dna2use:
    ...
    for rna_gene in rna2use:
      p_0 = prob_success_0.loc[rna_gene]; p_1 = prob_success_1.loc[rna_gene]
      f_0 = failure_0.loc[rna_gene];  f_1 = failure_1.loc[rna_gene]
      
      w = np.array([ np.log(p_1) - np.log(p_0)])
                     
                    
      z_train = R_train[rna_gene].values #np.vstack( ( np.log( R_train[rna_gene].values + 1e-12 ), np.log( 1.0-R_train[rna_gene].values + 1e-12 ) ) ).T
      z_val   = R_val[rna_gene].values #np.vstack( ( np.log( R_val[rna_gene].values + 1e-12 ), np.log( 1.0-R_val[rna_gene].values + 1e-12 ) ) ).T

      b = - special.gammaln( f_1 ) \
          + special.gammaln( f_0 ) \
          - f_0*np.log(1-p_0) + f_1*np.log(1-p_1)
