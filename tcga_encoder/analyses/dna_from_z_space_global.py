...
def main( data_location, results_location, alpha=0.02 ):
  ...
  dna_genes = dna.sum(0).sort_values(ascending=False)[:100].index.values

  for dna_gene in dna_genes:
    
    dna_values = dna[dna_gene]
    n_1 = dna_values.sum()
    ...
    for z_idx in range(n_z):
      ...
      p_values[z_idx]    = auc_p_value( aucs[z_idx], 0.5, se_aucs[z_idx], se_random )
    
    aucs_series = pd.Series( aucs, index = z_names, name=dna_gene)
    p_series = pd.Series( p_values, index = z_names, name=dna_gene)
    
    all_auc_series.append( aucs_series )
    all_p_series.append( p_series )
  
  auc_df = pd.concat( all_auc_series, axis=1 ).T
  p_df = pd.concat( all_p_series, axis=1 ).T

  auc_df.to_csv( dna_dir + "/global_auc_values.csv")  
  p_df.to_csv( dna_dir + "/global_p_values.csv")      
  
  f=pp.figure()
  ax1 = f.add_subplot(121)
  ax2 = f.add_subplot(122)
  ax1.hist( p_df.values.flatten(), bins = np.linspace(0,0.5,41), lw=2, histtype="step", normed=True, range=(0,0.5) )
  ax2.hist( auc_df.values.flatten(), bins = np.linspace(0.5,1,41), lw=2, histtype="step", normed=True, range=(0.5,1) )
  ...
  ax1 = f.add_subplot(121)
  ax2 = f.add_subplot(122)
  ax1.hist( p_df.values.flatten(), bins = np.linspace(0,0.5,101), lw=2, histtype="step", normed=True, range=(0,0.5) )
  ax2.hist( auc_df.values.flatten(), bins = np.linspace(0.5,1,101), lw=2, histtype="step", normed=True, range=(0.5,1) )
  ...
  pp.savefig( dna_dir + "/global_p_values_and_auc_scatter.png", fmt = "png", dpi=300)
  pp.close('all')
  return auc_df, p_df
  #pdb.set_trace()
