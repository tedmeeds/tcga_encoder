...
def main( data_location, results_location ):
  ...
  save_dir = os.path.join( results_path, "affinity_prop_with_z_local" )
  ...
  for t_idx in range(n_tissues):
    ...
    af_patients = AffinityPropagation(preference=-1500).fit(Z_cohort.values)
    patients_cluster_centers_indices = af_patients.cluster_centers_indices_
    kmeans_patients_labels = af_patients.labels_

    K_p = len(patients_cluster_centers_indices)
    
    af_z = AffinityPropagation(preference=-1500).fit(Z_cohort.values.T)
    z_cluster_centers_indices = af_z.cluster_centers_indices_
    kmeans_z_labels = af_z.labels_
    ...
    pp.savefig( save_dir + "/Z_af_%s.png"%(tissue_name), fmt="png", dpi=300, bbox_inches='tight')
    pp.close('all')
